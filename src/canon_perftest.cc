...
void OriginalCanonicalizePath(char* path, size_t* len, uint64_t* slash_bits) {
...
void CanonicalizePathEverythingMemmove(char* path, size_t* len, uint64_t* slash_bits) {
  // WARNING: this function is performance-critical; please benchmark
  // any changes you make to it.
  if (*len == 0) {
    return;
...
void CanonicalizePathOneLoop(char* path, size_t* len, uint64_t* slash_bits) {
  // WARNING: this function is performance-critical; please benchmark
  // any changes you make to it.
  if (*len == 0) {
    return;
...
template <typename F>
void run(F&& f, const char* name, std::string& pathCopies) {
  printf("%s\n", name);
...
int main() {
  ...
  run(OriginalCanonicalizePath, "OriginalCanonicalizePath", pathCopies);
  run(CanonicalizePathEverythingMemmove, "CanonicalizePathEverythingMemmove",
      pathCopies);
  run(CanonicalizePathOneLoop, "CanonicalizePathOneLoop", pathCopies);
}
